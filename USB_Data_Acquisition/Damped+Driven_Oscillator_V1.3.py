...
2024-11-25: V 1.3   Code update to run on newer numpy (NaN -> nan), compilation with Nuitka, window origin at (0,0).
                    "Driven Oscill." fit now reports separate angular drinving frequencies (top+bottom), and the average.
...
WindowTitle = 'Damped + Driven Oscillator  V 1.3'
...
global  fitline1, fitline2, fitline3, fitline4, arrow1, arrow2, arrow3, arrow4, dash1, dash2, span, span2,\
    text1, text2, text3, text4, textdamped, textexp, fitpar
...
MIN_SIZE = (1276, 645) # window minimum size
...
layout = [  [sg.Frame(layout=[[sg.Image(rel_to_abs_path('images/CMU-PHY_Logo_268px.png'), expand_x=True, expand_y=True ), 
            ...
            [sg.Frame(layout=[[sg.Column([[sg.Canvas(key='controls_cv')], [sg.Canvas(key='-CANVAS-', size=(520,None))]]),
                               sg.Column([[sg.Frame(layout=[[sg.Text('Serial Port:')],
                                                 [sg.Combo(ser_devices, default_value='', size=(15, 1), key='-SerPort-', readonly=True, pad=(5, (0,20)))],
                                                 [sg.Button('Detect Serial Port', size=14, key='-DetectSerialPort-')],
                                                 [sg.Button('Connect', size=14, key='-SerConnect-')]],
                                         title='Device', title_color='#FFC82E', size=(160,180)),
                                           sg.Frame(layout=[[sg.Column([[sg.Text(f'# of points\n(max. {NMax})')]], size=(110,40)),
                                                            ...
                                                            [sg.Column([[sg.Text('Acquiring a larger number\nof points takes more time.', font=('', 8), pad=(8,0))]])],
                                                            [sg.Column([[sg.Text('Filter Window')]], size=(110, 22)),
                                                             sg.Column([[sg.Input(FilterWindow0, size=(6, None), justification='right', 
                                                                                  key='-FilterWindow-' )]], 
                                                                       vertical_alignment='top')],
                                                            [sg.Column([[sg.Text('Larger filter window sizes\nresult in smoother curves.', font=('', 8), pad=(8,0))]])]                                                        
                                                           ], 
                                                    title="Acquisition Parameters", title_color='#FFC82E', size=(200,180))],
                                          [sg.Frame(layout=[[sg.Button('Acquire Data', key='-START-', size=12, disabled=True),
                                                            ...
                                                            [sg.Input(key='-SaveData-', enable_events=True, visible=False, size=10), sg.FileSaveAs(button_text='Save Data+Graph', file_types=(("TXT Files", "*.txt"),("ALL Files", "*.*")),
                                                                           size=12, key='-SaveData_Button-' ),
                                                             sg.Button('Fit Damped Osc.', key='-FitDamped-', size=12)]
                                                            ], 
                                                    title='Actions', title_color='#FFC82E', size=(370,96))],
                                          [sg.Frame(layout=[
                                                            [sg.Text('', size=(19,1), key='-T00-',font='Arial 10'),
                                                             sg.Text('', size=(5,1), key='-T01-', font='Arial 10 italic'),
                                                             sg.Text('', key='-T02-', text_color='#ffffff',font='Arial 10')],
                                                            [sg.Text('', size=(19,1), key='-T10-',font='Arial 10'),
                                                             sg.Text('', size=(5,1), key='-T11-', font='Arial 10 italic'),
                                                             sg.Text('', key='-T12-', text_color='#ffffff',font='Arial 10')],
                                                            [sg.Text('', size=(19,1), key='-T20-',font='Arial 10'),
                                                             sg.Text('', size=(5,1), key='-T21-', font='Arial 10 italic'),
                                                             sg.Text('', key='-T22-', text_color='#ffffff',font='Arial 10')],
                                                            [sg.Text('', size=(19,1), key='-T30-',font='Arial 10'),
                                                             sg.Text('', size=(5,1), key='-T31-', font='Arial 10 italic'),
                                                             sg.Text('', key='-T32-', text_color='#ffffff',font='Arial 10')],
                                                            [sg.Text('', size=(19,1), key='-T40-',font='Arial 10'),
                                                             sg.Text('', size=(5,1), key='-T41-', font='Arial 10 italic'),
                                                             sg.Text('', key='-T42-', text_color='#ffffff',font='Arial 10')],
                                                            [sg.Text('', size=(19,1), key='-T50-',font='Arial 10'),
                                                             sg.Text('', size=(5,1), key='-T51-', font='Arial 10 italic'),
                                                             sg.Text('', key='-T52-', text_color='#ffffff',font='Arial 10')]
                                                            ],
                                                    title='Analysis', title_color='#FFC82E', size=(370,190))]
                                          ])]], title='')],
            [sg.Frame(layout=[[sg.Text('Status: ', font=('', 9)), sg.Text('', key='-STATUS-', font=('', 9))]], title='', size=(1246,29), vertical_alignment='top')]
        ]

# Analysis text labels
OutText = [[['Ang. Driving Freq. (top):', 'ωdt  ='],
            ['Ang. Driving Freq. (bot):', 'ωdb  ='],
            ['Ang. Driving Freq. (avg):', 'ωd   ='],
            ['Top Amplitude:',      'A0  ='],
            ['Bottom Amplitude:',   'A   ='],
            ['Phase Shift: ',       'Δϕ  ='],
            ],
...
class Toolbar(NavigationToolbar2Tk):
    ...
    def set_message(self, s):   # Turn off live (x,y) display66
        pass
#%%
def onselect(t1, t2):
...
def fit_xdr(t, x, t0=0):
    ...
    print(f'p0 = {p0}')
    popt, pcov = curve_fit(xdr, t-t0, x, p0=p0, method='trf', bounds=((0, 0, -0.5*np.pi, -np.inf), (np.inf, np.inf, 2.5*np.pi, np.inf)))
    print(f'popt = {popt}\n\n')
...
def fit_driven():
    global data, tt, xx1, xx2, fitline1, fitline2, arrow1, arrow2, arrow3, arrow4, dash1, dash2, tmin, tmax, \
           text1, text2, text3, text4, fitpar
    ...
    if (len(data[:,0])>4) and (indmax-indmin>4):
        ...
        try:
            ...
            fitpar.clear()
            fitpar = {'A0': A0, 'A': A, 'phi': phi, 'omega_d': omega_d}
            ...
            window['-T02-'].update(value=f'({omega_d1.n:.3f} ± {omega_d1.s:.3f}) rad/s')
            window['-T12-'].update(value=f'({omega_d2.n:.3f} ± {omega_d2.s:.3f}) rad/s')
            window['-T22-'].update(value=f'({omega_d.n:.3f} ± {omega_d.s:.3f}) rad/s')
            window['-T32-'].update(value=f'({A0.n:.4f} ± {A0.s:.4f}) m')
            window['-T42-'].update(value=f'({A.n:.4f} ± {A.s:.4f}) m')
            window['-T52-'].update(value=f'({phi.n:.2f} ± {phi.s:.2f}) rad')
            window.refresh()
...
def fit_damped():
    global data, tt, xx1, xx2, fitline1, fitline2, fitline3, fitline4, arrow1, arrow2, arrow3, arrow4, dash1, dash2, tmin, tmax, \
           text1, text2, text3, text4, textexp, textdamped, fitpar
    if (tmin!=None) and (tmax!=None):
        indmin, indmax = np.searchsorted(data[:,0], (tmin, tmax))    
    else:
        indmin, indmax = (0,0)
...
def do_measurement():
    ...
    n = validate_int(values['-NPts-'], NMin, NMax)
    print('n=', n)
    ...
    if not np.isnan(n) and not np.isnan(w):
        try:
            ...
            update_status('Serial error. Check USB connection and click "Connect".', error=True)
            fig_canvas_agg.draw()
            window['-START-'].update(disabled=True)
            window['-SerConnect-'].update('Connect')
            return False
...
def save_data(filename):
    ...
    try:
        ...
        with open(filename, mode='w') as f:
            ...
            if len(fitpar.keys())>0:
                # write fit parameters
                f.write('#\n# Fit parameters:\n')
                for k in fitpar.keys():
                    f.write(f'# {k:8s} = {fitpar[k]}\n')
        
        # save graph
        fbase, fext = os.path.splitext(filename)
        fig.savefig(fbase+'.png', bbox_inches='tight', dpi=150)
...
window = sg.Window(WindowTitle, layout, font='Arial 10', finalize=True,  
                   enable_close_attempted_event=True, resizable=True, location=(0,0))
...
fit_text = ax2.text(0.98, 0.02, 'Click and drag in the bottom window to set the fit range. Click on white background to unset range.', fontsize=11, 
